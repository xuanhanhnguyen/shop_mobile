...
@section('content_header')
    <h5><a href="#" onclick="window.history.back()">Danh sách hóa đơn</a> / Cập nhật hóa đơn</h5>
@stop
        ...
        <form action="/admin/order/{{$order->id}}" method="POST">
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="khach_hang_id" required>
                    ...
                    @foreach($kh as $item)
                        <option {{$order->khach_hang_id == $item->id ? 'selected': ''}} value="{{$item->id}}">{{$item->ten_kh}}
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="san_pham_id" required>
                    <option value="">-------------chọn--------------</option>
                    @foreach($sp as $item)
                        <option {{$order->san_pham_id == $item->id ? 'selected': ''}} value="{{$item->id}}">{{$item->ten_sp}}
